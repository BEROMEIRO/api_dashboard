...
if response.status_code == 200:
    ...
    if items:  # Verifica se há itens retornados
        ...
        if "name" in df.columns and "description" in df.columns and "price" in df.columns and "stock" in df.columns:
            fig1 = px.scatter(df, 
                              x=df["name"].apply(len), 
        ...
        if "created_at" in df.columns and "stock" in df.columns:
        ...
        # Exibe uma tabela vazia com a estrutura definida
        df = pd.DataFrame(columns=["name", "description", "price", "category", "created_at"])
        st.write(df)
